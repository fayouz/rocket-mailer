...
class HealthChecker
{
    ...
    {
        return \sprintf('mailbox:%s:%s', $mailbox->getId(), $protocol);
    }
    ...
    public function checkAll(): bool
    {
        ...
        try {
            ...
            foreach ($this->mailboxes->findBy(['enabled' => true]) as $mailbox) {
                $start = hrtime(true);
                $result = $this->connector->test($mailbox);
                $this->record($existing, $seen, self::mailboxCheckId($mailbox, 'smtp'), $result['smtp']['ok'], $result['smtp']['message'], $start);
                if (null !== $result['imap']) {
                    $this->record($existing, $seen, self::mailboxCheckId($mailbox, 'imap'), $result['imap']['ok'], $result['imap']['message'], null);
                }
            }

            // LDAP turned off, mailbox disabled or deleted: forget its results.
