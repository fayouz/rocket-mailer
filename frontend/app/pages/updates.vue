...
const cronLine = computed(() => `* * * * * cd ${(update.value?.methods.script.script ?? '/srv/rocket-mailer/deploy/update.sh').replace(/\/deploy\/[^/]+$/, '')}/backend && php bin/console app:update:run`)
  ...
  <UDashboardPanel id="updates">
    ...
    <template #body>
      <div v-if="update" class="flex max-w-3xl flex-col gap-6">
        ...
        <UPageCard title="Méthode de mise à jour" description="Selon l’installation de Rocket Mailer." icon="i-lucide-settings-2" variant="subtle">
          <URadioGroup
            :model-value="method"
            :items="methodItems"
        ...
        <!-- Manual -->
        <UPageCard v-else title="Mettre à jour" description="Sur le serveur, dans le dossier de Rocket Mailer :" icon="i-lucide-terminal" variant="subtle" data-testid="manual-update">
