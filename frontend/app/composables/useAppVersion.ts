...
export function useAppVersion() {
  ...
  const auth = useAuth()
  const api = useApi()
  const apiVersion = useState<AppVersionInfo | null>('rm_api_version', () => null)
  const update = useState<UpdateStatus | null>('rm_update_status', () => null)
  const loaded = useState('rm_version_loaded', () => false)
