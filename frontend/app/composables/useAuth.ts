...
const TOKEN_COOKIE = 'rm_token'
const TOKEN_TTL_SECONDS = 3600

export function useAuth() {
  const config = useRuntimeConfig()
  const token = useState<string | null>('rm_token', () => useCookie<string | null>(TOKEN_COOKIE).value ?? null)
  ...
  // Embed tokens stay in memory: third-party iframes cannot rely on cookies.
  const embedToken = useState<string | null>('rm_embed_token', () => null)
  const me = useState<Me | null>('rm_me', () => null)
